...
public class MainClass {
	...
	public static void printFinalPath(AStar aStar) {
		...
		//Imprimimos las metricas del numero de nodos del path y del % de acierto a la hora de explorar
	...
	public static void printMetrics(AStar aStar, double executionTime) {
		...
		//Imprimos las metricas del tiempo de trabajo y del numero de nodos explorados, adyacentes y generados
		...
		System.out.println("\u001B[33m"+String.format("%-8s",aStar.getGoalNode().getTrabajadores().get(0).getNombre()) + " ha trabajado " + (int)Math.round(aStar.getGoalNode().getTrabajadores().get(0).getMinutosTrabajados()) + " minutos que son " + String.format("%.2f", aStar.getGoalNode().getTrabajadores().get(0).getMinutosTrabajados()/60) + " horas." + "\u001B[0m");
	}
}